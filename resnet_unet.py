...
__all__ = ['resnet18_unet', 'resnet34_unet', 'resnet50_unet']
...
class SResUnet(nn.Module):
    # Shallow Unet with ResNet18 or ResNet34
...
class DResUnet(nn.Module):
    # Deep Unet with ResNet50, ResNet101 or ResNet152
    def __init__(self, encoder, *, pretrained=False, out_channels=2):
        super().__init__()
        self.encoder = encoder(pretrained=pretrained)
        self.encoder_layers = list(self.encoder.children())
...
def resnet18_unet(output_dim: int=1, pretrained: bool=False):
    return SResUnet(resnet.resnet18, pretrained=pretrained, out_channels=output_dim)


def resnet34_unet(output_dim: int=1, pretrained: bool=False):
    return SResUnet(resnet.resnet34, pretrained=pretrained, out_channels=output_dim)


def resnet50_unet(output_dim: int=1, pretrained: bool=False):
    return DResUnet(resnet.resnet50, pretrained=pretrained, out_channels=output_dim)
